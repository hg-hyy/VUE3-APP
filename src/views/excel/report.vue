<template>
  ...
  <div class="excel mx-3 shadow-lg p-3 mb-5 bg-body rounded">
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="excelmodal">Modal title</h5>
            <button
              type="button"
              class="btn-close"
          ...
          <div class="modal-body">wraetryjtukyiul</div>
      ...
      <div style="min-height: 550px; padding: 0 20px">
        <div class="excel-header">
          ...
          >
            ...
            >
              {{ item.template_table_name }}3345465678
            </option>
          </select>
          <button
          ...
          <button class="btn btn-info" @click="exportExcel">下载模板</button>
          <input
            type="checkbox"
            id="checkbox"
            v-show="checkBoxMutiplian"
            v-model="picSubmitShow"
          />
          <label for="checkbox">将图片保存到系统中</label>
          <div style="margin: 0 auto; display: inline-block; float: right">
            <button
              :disabled="exportPower"
              class="btn btn-info"
              @click="saveDataBase"
            >
              导入
            ...
            <button
              :disabled="canclePower"
              class="btn btn-info"
              @click="cancleTemplate"
            >
              关闭
            </button>
          </div>
        ...
        >
          <p
            ...
            :key="index"
            style="color: red; height: 20px; line-height: 14px; margin: 0"
          >
            {{ item }}
          </p>
        </div>
        ...
        <p style="text-align: center; line-height: 50px; margin-bottom: 0">
          (预览数据)
        </p>
      </div>
    </div>
    ...
    <div v-if="centerDialogVisibleTwo" title="导入结果" width="40%" center>
      <p style="text-align: center">成功条数：{{ successCount }}</p>
      <p style="text-align: center">失败条数：{{ failCount }}</p>
      <span class="div-footer">
        <button @click="centerDialogVisibleTwo = false">关闭</button>
        <button v-show="exportFale" type="primary" @click="exportFailData">
          导出失败数据
        </button>
      </span>
    </div>
  ...
  methods: {
        ...
        uploadPicToQn(arrimg)
          .then((res) => {
            for (let h = 0; h < arrimg.length; h++) {
              ...
              this.submitLastValue[this.twoArr][
                this.checked[this.oneArr]
              ] = this.submitLastValue[this.twoArr][
                this.checked[this.oneArr]
              ].replace(arrimg[h], res.data[h].pic_name);
              this.count += 1;
            }
            this.debugCount += 1;
            if (
              this.oneArr === this.checked.length - 1 &&
              this.twoArr === this.submitLastValue.length - 1
            ) {
              // 判断src的值是否转换完成
              this.saveResult();
      ...
      FileReader.prototype.readAsBinaryString = function (f) {
        ...
        reader.onload = function (e) {
          ...
          for (const i in wb.Sheets[wb.SheetNames[0]]) {
            if (
              wb.Sheets[wb.SheetNames[0]][i]["z"] &&
              wb.Sheets[wb.SheetNames[0]][i]["z"] === "m/d/yy"
            ) {
              wb.Sheets[wb.SheetNames[0]][i]["v"] = that.formatDate(
                parseInt(wb.Sheets[wb.SheetNames[0]][i]["v"]),
                "-"
              );
            }
          }
          ...
          that.arr1.unshift(" "); // 字段不存在或者没有值时，则将它的值设置为空
          for (let i = 0; i < outdata.length; i++) {
          ...
          // 由于数组和对象是引用类型，在赋值时候会将地址也一起赋值，当改变一个值得时候会将另外一个值也改变，
          //所以先转为字符串再转回来（基本类型只会传值不会传地址
    ...
    changeListValue(index) {
      ...
      for (var i = 0; i < this.tableData.length; i++) {
        this.tableData[i][this.arr[index]] = this.copyTableData[i][
          this.changeList[this.arr[index]]
        ];
      }
      this.resultData();
    },
    ...
    saveDataBase() {
      ...
      for (let j = 0; j < this.copyTableDataTwo.length; j++) {
        ...
        for (let i = 0; i < this.arr.length; i++) {
          if (
            this.copyTableDataTwo[j][this.changeList[this.arr[i]]] ||
            this.copyTableDataTwo[j][this.changeList[this.arr[i]]] === 0
          ) {
            obj[this.arr[i]] = this.copyTableDataTwo[j][
              this.changeList[this.arr[i]]
            ];
          } else {
            obj[this.arr[i]] = "";
          }
    ...
    saveResult: function () {
      ...
      for (let i = 0; i < this.templateAllData.length; i++) {
        for (let j = 0; j < this.submitLastValue.length; j++) {
          if (
            this.templateAllData[i].attribute_not_null === true &&
            !this.submitLastValue[j][this.templateAllData[i].attribute_name] &&
            this.submitLastValue[j][this.templateAllData[i].attribute_name] !==
              0
          ) {
            //
            console.log(this.templateAllData[i].attribute_name + "不能为空"); //
            this.loading = false; //
            return this.errorList.push(
              this.templateAllData[i].attribute_name + "不能为空"
            );
          }
          if (
            (this.templateAllData[i].attribute_not_null === "true" ||
              this.templateAllData[i].attribute_not_null === true) &&
            this.templateAllData[i].attribute_type === "int4" &&
            !/(^[0-9]\d*$)/.test(
              this.submitLastValue[j][this.templateAllData[i].attribute_name]
            )
          ) {
            // 判断长度
            this.submitLastValue[j][
              this.templateAllData[i].attribute_name
            ].toString().length > this.templateAllData[i].attribute_length; //
            console.log(
              this.templateAllData[i].attribute_name + "传输类型应为整形"
            ); //
            this.loading = false; //
            return this.errorList.push(
              this.templateAllData[i].attribute_name + "传输类型应为整形"
          ...
          if (
            this.templateAllData[i].attribute_type === "numeric" &&
            this.submitLastValue[j][this.templateAllData[i].attribute_name] &&
            typeof this.submitLastValue[j][
              this.templateAllData[i].attribute_name
            ] !== "number"
          ) {
            //
            console.log(
              this.templateAllData[i].attribute_name + "传输类型为数字"
            ); //
            this.loading = false; //
            return this.errorList.push(
              this.templateAllData[i].attribute_name + "传输类型为数字"
          ...
          if (
            this.templateAllData[i].attribute_type === "date" &&
            this.submitLastValue[j][this.templateAllData[i].attribute_name]
          ) {
            this.submitLastValue[j][
              this.templateAllData[i].attribute_name
            ...
            if (
              ...
              //
              console.log(this.templateAllData[i].attribute_name + "格式有误"); //
              this.loading = false; //
              return this.errorList.push(
                this.templateAllData[i].attribute_name + "格式有误"
              );
            }
          }
          if (this.templateAllData[i].attribute_type === "bool") {
            ...
            console.log(
              this.templateAllData[i].attribute_name + "传输类型为布尔值"
            ); //
            this.loading = false; //
            return this.errorList.push(
              this.templateAllData[i].attribute_name + "传输类型为布尔值"
    ...
    exportExcel() {
      ...
      this.JSONToExcelConvertor(this.submitLastValue, "report", this.arr, [
        "id",
      ]);
    },
      ...
      if (title) {
        ...
        for (const i in arrData[0]) {
          ...
          for (let j = 0; j < this.templateAllData.length; j++) {
            if (
              i === this.templateAllData[j].attribute_name &&
              this.templateAllData[j].attribute_not_null === true
            ) {
              num += 1;
            }
          }
          if (num === 0) {
            ...
            // 导出时必填的有*号
            row +=
              "<th align='center'><span style='color: red'>" +
              "*" +
              "</span>" +
              i +
              "</th>";
    ...
    tableToExcel: function (jsonData) {
      ...
      str += "</tr>"; // 循环遍历，每行加入tr标签，每个单元格加td标签
      for (let i = 0; i < jsonData.length; i++) {
        str += "<tr>";
        for (const item in jsonData[i]) {
          // 增加\t为了不让表格显示科学计数法或者其他格式
      ...
      window.location.href =
        "data:application/vnd.ms-excel;base64," + this.base64(template);
    },
    base64: function (s) {
...
<style scoped>
.excel1 {
  display: inline-block; /*position: relative;*/ /*right: 90px;*/
}
.input-file {
  display: none;
}
